...
export function Button({
  ...
  return (
    <button
      className={cn(
        "inline-flex cursor-pointer items-center justify-center rounded-full px-5 py-2.5 text-sm font-medium tracking-wide transition disabled:cursor-not-allowed disabled:opacity-50",
...
export function Field({
  ...
  return (
    <label htmlFor={htmlFor} className="block space-y-1.5">
      ...
      {error ? <span className="block text-sm text-rose-800">{error}</span> : null}
    </label>
  );
}

const control =
  "w-full rounded-2xl border border-gold/30 bg-white px-4 py-3 text-base text-ink outline-none ring-forest/20 placeholder:text-forest/40 focus:ring-2 aria-[invalid=true]:border-rose-400 aria-[invalid=true]:ring-2 aria-[invalid=true]:ring-rose-200";
